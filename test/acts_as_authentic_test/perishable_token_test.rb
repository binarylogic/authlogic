...
module ActsAsAuthenticTest
  class PerishableTokenTest < ActiveSupport::TestCase
    def test_perishable_token_valid_for_config
      ...
      assert_equal 10.minutes.to_i, klass.perishable_token_valid_for
      ...
      klass.perishable_token_valid_for = 1.hour
      assert_equal 1.hour.to_i, klass.perishable_token_valid_for
      klass.perishable_token_valid_for 10.minutes
      assert_equal 10.minutes.to_i, klass.perishable_token_valid_for
    end
    ...
    def test_disable_perishable_token_maintenance_config
      ...
      assert !klass.disable_perishable_token_maintenance
      assert !User.disable_perishable_token_maintenance
      assert !Employee.disable_perishable_token_maintenance

      klass.disable_perishable_token_maintenance = true
      assert klass.disable_perishable_token_maintenance
      klass.disable_perishable_token_maintenance false
      assert !klass.disable_perishable_token_maintenance
    end
    ...
    def test_find_using_perishable_token_when_perished_with_smaller_deadline
      klass = testable_user_class
      klass.perishable_token_valid_for = 1.minute

      ben = klass.find(users(:ben).id)
      ActiveRecord::Base.connection.execute("UPDATE users set updated_at = '#{2.minutes.ago.to_s(:db)}' where id = #{ben.id}")
      assert_nil klass.find_using_perishable_token(ben.perishable_token)
    end
    ...
    def test_find_using_perishable_token_when_passing_threshold
      ...
      assert_nil klass.find_using_perishable_token(ben.perishable_token, 5.minutes)
      assert_equal ben, klass.find_using_perishable_token(ben.perishable_token, 20.minutes)
    end
