...
module ActsAsAuthenticTest
  class PasswordTest < ActiveSupport::TestCase
    ...
    def test_validates_length_of_password
      ...
      assert_equal ["is too short (minimum is 4 characters)"], u.errors[:password]
      assert_equal ["is too short (minimum is 4 characters)"], u.errors[:password_confirmation]
    end
    ...
    def test_validates_confirmation_of_password
      ...
      if ActiveModel.respond_to?(:version) and ActiveModel.version.segments.first >= 4
        assert_equal ["doesn't match Password"], u.errors[:password_confirmation]
      else
        assert_equal ["doesn't match confirmation"], u.errors[:password]
      end
    end
